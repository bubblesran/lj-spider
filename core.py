...
BASE_URL = u"http://cq.lianjia.com/"
...
def GetCommunityByRegionlist(regionlist): #orginal code: regionlist=[u'xicheng']
...
def get_house_percommunity(communityname):
    ...
    for page in range(total_pages):
        if page > 0:
            url_page = BASE_URL + u"ershoufang/pg%drs%s/" % (page+1, urllib.parse.quote(communityname.encode('utf8')))
...
def get_sell_percommunity(communityname):
    ...
    for page in range(total_pages):
        if page > 0:
            url_page = BASE_URL + u"chengjiao/pg%drs%s/" % (page+1, urllib.parse.quote(communityname.encode('utf8')))
            source_code = misc.get_source_code(url_page)
            soup = BeautifulSoup(source_code, 'lxml')
        i = 0
...
def get_community_perregion(regionname): #orginal code: regionname=u'xicheng'
...
def get_rent_percommunity(communityname):
    ...
    for page in range(total_pages):
        if page > 0:
            url_page = BASE_URL + u"rent/pg%drs%s/" % (page+1, urllib.parse.quote(communityname.encode('utf8')))
            source_code = misc.get_source_code(url_page)
            soup = BeautifulSoup(source_code, 'lxml')
        i = 0
